	...
	<div class="featured" id="post-<?php the_ID(); ?>"> 
		...
		<?php if ( get_post_meta($post->ID, 'image', true) ) { ?> <!-- DISPLAYS THE IMAGE URL SPECIFIED IN THE CUSTOM FIELD -->
			...
			<div class="featured-img" style="background:url(<?php echo bloginfo('template_url'); ?>/thumb.php?src=<?php echo get_post_meta($post->ID, "image", $single = true); ?>&amp;h=200&amp;w=350&amp;zc=1&amp;q=95) top left no-repeat;">
				<?php if ($counter == 1) { ?><!--<div id="ribbon"><img src="<?php bloginfo('stylesheet_directory'); ?>/styles/<?php echo "$style_path"; ?><?php _e('/ribbon.png', 'arsofia' ); ?>" alt="Featured"  /></div> --><?php } ?> <!-- SHOW "FEATURED" RIBBON FOR FIRST POST ONLY -->
			</div>				
		...
		<p style="margin-bottom:0px !important;"><span class="continue"><a title="<?php the_title(); ?>" href="<?php the_permalink() ?>"><?php _e('Continue Reading', 'arsofia' ); ?></a></span></p>
...
<div id="featured-th">
	...
	<ul class="idTabs">
		...
		<?php if ( get_post_meta($post->ID, 'image', true) ) { ?> <!-- DISPLAYS THE IMAGE URL SPECIFIED IN THE CUSTOM FIELD -->
			...
			<li <?php if ($counter == 5) { ?>class="last"<?php } ?>><a href="#post-<?php the_ID(); ?>"><img width="100" height="57" src="<?php echo bloginfo('template_url'); ?>/thumb.php?src=<?php echo get_post_meta($post->ID, "image", $single = true); ?>&amp;h=57&amp;w=100&amp;zc=1&amp;q=95" alt=""/></a></li>			
